...
//            groupModels.add(groupModel1);
        ...
        file.putFile(uri).addOnSuccessListener(taskSnapshot-> file.getDownloadUrl().addOnSuccessListener(uri->{
            GroupModel model = new  GroupModel(groupModel,uri.toString(),Name,"Is a Group",false);
            users.getReference().child("Group").child(Name + Objects.requireNonNull(auth.getUid())).push().setValue(model).addOnSuccessListener(unused -> {
                if (bar.isShowing()) {
                    bar.dismiss();
                    getActivity().finish();
